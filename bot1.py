...
class BlumTod:
    def __init__(self, id, query, proxies, config: Config):
        ...
        if len(self.proxies) > 0:
            ...
            self.ses = 
# Creating a custom SSL context for secure communication
import ssl
ssl_context = ssl.create_default_context()

httpx.AsyncClient(verify=ssl_context, transport=transport, timeout=1000)
        else:
            self.ses = 
# Creating a custom SSL context for secure communication
import ssl
...
httpx.AsyncClient(verify=ssl_context, timeout=1000)
        self.headers = {
...
httpx.AsyncClient(verify=ssl_context, transport=transport)
